...
import { ARCHIVE_ROLES, CRM_OPERATOR_ROLES, MASTER_DATA_ROLES, hasRole } from "@/lib/auth/permissions";
...
function sortHref(state: TableState, sort: CustomerSort) {
...
function exportHref(state: TableState) {
  const params = new URLSearchParams();
  if (state.query) params.set("q", state.query);
  if (state.segment !== "all") params.set("segment", state.segment);
...
function CustomerExcelActions({ href }: { href: string }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      <Button variant="outline" render={<a href={href} />} nativeButton={false}>
      ...
      <Dialog>
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Upload XLSX dengan kolom Nama, Jenis customer, dan minimal satu kontak.</DialogDescription>
          </DialogHeader>
          <form action={importCustomersAction}>
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="customer-import-file">File import</FieldLabel>
                <FilePicker
                  id="customer-import-file"
                  name="file"
                  accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                  emptyLabel="Belum ada file Excel"
                />
...
async function CustomersTableSection({ searchParams }: { searchParams: CustomerSearchParams }) {
  ...
  const canImportExport = Boolean(actor && hasRole(actor.role, MASTER_DATA_ROLES));
            ...
            >
              <TableHeader className="sticky top-0 bg-muted">
                <TableRow className="hover:bg-muted">
                  ...
                  <SortableTableHead label="No. customer" href={sortHref(state, "customerNo")} active={sort === "customerNo"} direction={direction} />
                  <SortableTableHead label="Customer" href={sortHref(state, "name")} active={sort === "name"} direction={direction} className="min-w-56" />
                  ...
                  <SortableTableHead label="Peluang" href={sortHref(state, "opportunities")} active={sort === "opportunities"} direction={direction} />
