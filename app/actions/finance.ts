...
const expenseSchema = z.object({ id: z.string().cuid().optional(), purpose: z.string().trim().min(2, "Keperluan minimal 2 karakter.").max(500), spentAt: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Tanggal tidak valid."), amount: z.coerce.number().finite().positive("Nominal harus lebih dari 0."), paymentMethod: z.enum(EXPENSE_METHODS) });
const idSchema = z.string().cuid();
const PATH = "/keuangan/pengeluaran";

function input(formData: FormData) { return { id: formData.get("id"), purpose: formData.get("purpose"), spentAt: formData.get("spentAt"), amount: formData.get("amount"), paymentMethod: formData.get("paymentMethod") }; }
...
export async function createExpenseAction(formData: FormData) {
  return runRedirectingAction(PATH, async () => {
    ...
    await getPrismaClient().$transaction(async (tx) => { const item = await tx.expense.create({ data: { ...parsed.data, spentAt: date(parsed.data.spentAt), amount: new Prisma.Decimal(parsed.data.amount), createdById: actor.id } }); await audit(tx, actor.id, item.id, "EXPENSE_CREATED", ["purpose", "spentAt", "amount", "paymentMethod"]); });
...
export async function updateExpenseAction(formData: FormData) {
  return runRedirectingAction(PATH, async () => {
    ...
    await getPrismaClient().$transaction(async (tx) => { const current = await tx.expense.findUnique({ where: { id }, select: { createdById: true, reimbursedAt: true } }); if (!current || current.createdById !== actor.id) throw new UserFacingError("Anda hanya dapat mengubah pengeluaran sendiri."); if (current.reimbursedAt) throw new UserFacingError("Pengeluaran yang sudah diganti tidak dapat diubah."); await tx.expense.update({ where: { id }, data: { purpose: parsed.data.purpose, spentAt: date(parsed.data.spentAt), amount: new Prisma.Decimal(parsed.data.amount), paymentMethod: parsed.data.paymentMethod } }); await audit(tx, actor.id, id, "EXPENSE_UPDATED", ["purpose", "spentAt", "amount", "paymentMethod"]); });
