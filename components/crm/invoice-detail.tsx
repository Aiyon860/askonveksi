...
function PaymentSummary({ payment, methods, canRecord, onRecorded }: { payment: Payment; methods: Array<{ id: string; name: string }>; canRecord: boolean; onRecorded: () => Promise<void> }) {
  ...
  return <section className="flex flex-col gap-3" aria-label="Pembayaran invoice">
    ...
    {payment.terms.map((term) => <div key={term.id} className="rounded-lg border p-3 text-sm">
      ...
      <dl className="mt-3 grid gap-2 sm:grid-cols-2"><Info label="Nominal" value={formatCurrency(term.amount)} /><Info label={`Deadline Termin ${term.position + 1}`} value={formatDate(term.dueAt)} />{term.transaction ? <><Info label="Metode pembayaran" value={term.transaction.paymentMethod?.name ?? "-"} /><Info label="Dibayar pada" value={formatDate(term.transaction.paidAt, true)} /><Proof transaction={term.transaction} /></> : null}</dl>
